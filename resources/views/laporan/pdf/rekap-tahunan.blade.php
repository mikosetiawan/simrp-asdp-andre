{{-- resources/views/laporan/pdf/rekap-tahunan.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
...
<div class="header">
  ...
  <p>REKAPITULASI PENDAPATAN TAHUNAN — LINTAS MERAK–BAKAUHENI</p>
</div>
<div class="meta">
  <div class="meta-item"><label>Tahun</label><span>{{ $tahun }}</span></div>
...
<div class="content">
  <table>
    <thead><tr><th>Bulan</th><th class="td-right">Total Trip</th><th class="td-right">Total Penumpang</th><th class="td-right">Total Pendapatan (Rp)</th></tr></thead>
    <tbody>
      @php $namaBulan = ['','Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember']; @endphp
      @foreach($per_bulan as $bln => $dataBulan)
      <tr>
        <td>{{ $namaBulan[$bln] }}</td>
        <td class="td-right">{{ number_format($dataBulan['trip']) }}</td>
        <td class="td-right">{{ number_format($dataBulan['penumpang']) }}</td>
        <td class="td-right"><strong>Rp {{ number_format($dataBulan['pendapatan'], 0, ',', '.') }}</strong></td>
      </tr>
      @endforeach
    </tbody>
    <tfoot>
