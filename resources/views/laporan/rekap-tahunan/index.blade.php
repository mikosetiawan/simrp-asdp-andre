...
@section('breadcrumb', 'Laporan → Rekap Tahunan')
@section('content')
<div class="space-y-5">
    {{-- Filter --}}
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            @can('laporan.export')
            <a href="{{ route('laporan.rekap-tahunan.pdf', ['tahun' => $tahun, 'regu_id' => $regu_id]) }}"
               class="bg-red-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-red-700 transition">📄 PDF</a>
            <a href="{{ route('laporan.rekap-tahunan.excel', ['tahun' => $tahun, 'regu_id' => $regu_id]) }}"
    ...
    {{-- Chart --}}
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 class="font-semibold text-gray-800 mb-4">📈 Tren Pendapatan Bulanan</h3>
        <canvas id="chartTahunan" height="80"></canvas>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    @forelse($data['per_bulan'] as $bln => $dataBulan)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium">{{ $namaBulan[$bln] }}</td>
                        <td class="px-4 py-3 text-right">{{ number_format($dataBulan['trip']) }}</td>
                        <td class="px-4 py-3 text-right">{{ number_format($dataBulan['penumpang']) }}</td>
                        <td class="px-4 py-3 text-right font-semibold">Rp {{ number_format($dataBulan['pendapatan'], 0, ',', '.') }}</td>
                    </tr>
                    @empty
...
@push('scripts')
<script>
const perBulan = @json($data['per_bulan']);
const namaBulan = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Ags','Sep','Okt','Nov','Des'];
const labels = Object.keys(perBulan).map(m => namaBulan[m-1]);
const pendapatan = Object.values(perBulan).map(h => h.pendapatan);
